...
image_geom = cil.framework.ImageGeometry(voxel_num_x=image_shape[2], voxel_num_y=image_shape[1],
                                         voxel_num_z=image_shape[0], voxel_size_x=2.78, voxel_size_y=2.98,
                                         voxel_size_z=3.12)
...
img_qual_callback = imgq.ImageQualityCallback(
    ref_image, tb_summary_writer, roi_mask_dict=roi_image_dict, metrics_dict={'MSE': MSE, 'MAE': MAE, 'PSNR': PSNR},
    statistics_dict={
        'MEAN': (lambda x: x.mean()), 'STDDEV': (lambda x: x.std()), 'MAX': (lambda x: x.max()),
        'COM': (lambda x: np.array([3, 2, 1]))}, post_smoothing_fwhms_mm_list=[5., 8.])
