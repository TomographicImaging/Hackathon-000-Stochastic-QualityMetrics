...
from cil.optimisation.functions import IndicatorBox, LeastSquares, TotalVariation, ZeroFunction
...
#%%
A = ProjectionOperator(image_geometry=ig, acquisition_geometry=absorption.geometry)
...
top.fill(np.asarray(ground_truth.array > 0.8 * ground_truth.max(), dtype=np.float32))
bottom.fill(np.asarray(np.invert(ground_truth.array < 0.4 * ground_truth.max()), dtype=np.float32))
#%%
show2D([ground_truth, top, bottom], num_cols=3)
#%%
# setup a 2 binary ROI images
roi_image_dict = {'top': top, 'bottom': bottom}
...
img_qual_callback = imgq.ImageQualityCallback(
    ground_truth,
    tb_summary_writer,
    roi_mask_dict=roi_image_dict,
    metrics_dict={'NRMSE': NRMSE, 'MSE': MSE, 'MAE': MAE, 'PSNR': PSNR},
    statistics_dict={
        'MEAN': (lambda x: x.mean()), 'STDDEV': (lambda x: x.std()), 'MAX': (lambda x: x.max()),
        'COM': (lambda x: np.array([3, 2, 1]))},
                                                                                                 # post_smoothing_fwhms_mm_list = [5., 8.]
)

#%%
algo.run(100, callbacks=[callbacks.ProgressCallback(), img_qual_callback])
# %%
